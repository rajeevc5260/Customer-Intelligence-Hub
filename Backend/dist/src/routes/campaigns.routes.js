...
import { campaigns, campaignAudience, campaignResponses, clients, } from "../db/schema.js";
...
campaignsRouter.post("/", requireAuth, requireAnyRole("leader", "admin", "ops"), async (req, res) => {
    ...
    if (Array.isArray(audienceUserIds) && audienceUserIds.length > 0) {
        const rows = audienceUserIds.map((userId) => ({
...
campaignsRouter.post("/:id/respond", requireAuth, requireAnyRole("consultant", "leader"), async (req, res) => {
...
campaignsRouter.get("/:id/responses", requireAuth, requireAnyRole("leader", "admin", "executive"), async (req, res) => {
    const { id } = req.params;
    const data = await db
        .select()
        .from(campaignResponses)

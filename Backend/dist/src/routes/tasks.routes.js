...
tasksRouter.get("/", requireAuth, async (req, res) => {
    ...
    const data = await filteredDataQuery
        ...
        .offset(offset);
    // COUNT QUERY
    const baseCountQuery = db.select({ count: sql `count(*)` }).from(tasks);
...
----------------------------- */
tasksRouter.get("/by-opportunity/:opportunityId", requireAuth, async (req, res) => {
...
----------------------------- */
tasksRouter.get("/by-insight/:insightId", requireAuth, async (req, res) => {
...
----------------------------- */
tasksRouter.post("/:id/status", requireAuth, async (req, res) => {
    const { id } = req.params;
